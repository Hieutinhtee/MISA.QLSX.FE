...
const columns = ref([
    ...
    { key: "contractStatus", name: "Trạng thái", type: "custom", width: 130, pinned: true },
...
const getStatusColor = (status) => {
    switch (status?.toLowerCase()) {
        case "active": return "#52c41a";      // Xanh lá đậm (Success)
        case "signed": return "#1890ff";      // Xanh dương (Processing)
        case "draft": return "#8c8c8c";       // Xám (Draft)
        case "expired": return "#faad14";     // Cam (Warning)
        case "terminated": return "#f5222d";  // Đỏ (Error)
...
<template>
    <div class="content d-flex flex-1 flex-column">
        ...
        <div class="content__body d-flex flex-1">
            ...
            >
                ...
                <template #row-actions="{ row }">
                    <div class="d-flex gap-8">
                        ...
                        <ms-button
                            v-if="row.contractStatus !== 'terminated' && row.contractStatus !== 'expired'"
                            type="danger-outline"
                            size="small"
            ...
            >
                Bạn có chắc chắn muốn chấm dứt hợp đồng <b>{{ contractToTerminate?.contractCode }}</b> của nhân viên <b>{{ contractToTerminate?.employeeName }}</b> không?
            </ms-alert>
        </div>
    </div>
</template>
