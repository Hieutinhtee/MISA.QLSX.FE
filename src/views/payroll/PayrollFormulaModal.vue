...
const childrenMap = computed(() => {
    ...
    map["(3)"] = items.value.filter(
        (i) =>
            i.formulaComponent === "bonus" ||
            (i.itemType === "addition" &&
                !["base_salary", "allowance"].includes(i.formulaComponent)),
    );
    map["(4)"] = items.value.filter(
        (i) =>
            i.formulaComponent === "penalty" ||
            (i.itemType === "deduction" && !["insurance", "tax"].includes(i.formulaComponent)),
    );
    ...
    >
        <div v-if="props.payroll" class="formula-container">
            ...
            <Row :gutter="16" class="mb-24">
                ...
                <Col :span="6">
                    <Card size="small" class="summary-card">
                        ...
                        >
                            <template #prefix><CalculatorOutlined /></template>
                        </Statistic>
                    </Card>
                </Col>
                <Col :span="6">
            ...
            <Descriptions bordered :column="2" size="small" class="mb-24">
                ...
                <Descriptions.Item label="Công chuẩn"
                    >{{ props.payroll.workingDaysStandard }} ngày</Descriptions.Item
                >
                <Descriptions.Item label="Công thực tế"
                    >{{ props.payroll.workingDaysActual }} ngày</Descriptions.Item
                >
            </Descriptions>
            ...
            >
                <template #bodyCell="{ column, record }">
                    ...
                    <template v-if="column.key === 'itemName'">
                        <span :class="record.hasChildren ? 'font-bold' : ''">{{
                            record.itemName
                        }}</span>
                    </template>
                    <template v-if="column.key === 'amount'">
                        <span
                            :class="
                                record.group === 'deduction' || record.group === 'tax-ins'
                                    ? 'text-danger'
                                    : 'text-success'
                            "
                        >
                ...
                <template #expandedRowRender="{ record }">
                    <div class="expanded-items">
                        <template
                            v-if="
                                childrenMap[record.indexLabel] &&
                                childrenMap[record.indexLabel].length
                            "
                        >
                            <div
                            ...
                            >
                                <span
                                    >• {{ child.itemName }}
                                    <small v-if="child.note">({{ child.note }})</small></span
                                >
                                <span
                                    :class="
                                        record.group === 'deduction' || record.group === 'tax-ins'
                        ...
                        <div v-else class="text-muted text-center italic">
            ...
            <div class="formula-explanation mt-24">
                ...
                <div class="explanation-step">
                    ...
                    <div class="step-desc">
                        Lương Gross = <span class="ref">(1)</span> Lương cơ bản +
                        <span class="ref">(2)</span> Phụ cấp + <span class="ref">(3)</span> Cộng
                        thêm - <span class="ref">(4)</span> Khấu trừ
                    </div>
                    <div class="step-calc">
                        =
                        {{
                            formatCurrency(
                                items.find((i) => i.formulaComponent === "base_salary")?.amount ||
                                    0,
                            )
                        }}
                        ...
                        <span class="font-bold"
                            >{{ formatCurrency(props.payroll.grossSalary) }} ₫</span
                        >
                    </div>
                    <div class="step-note">
                        Lương cơ bản tính pro-rate theo số ngày công thực tế trong hợp đồng. Phụ cấp
                        từ hợp đồng (cố định hoặc % lương). Cộng thêm/trừ từ chấm công (bonus, OT,
                        phạt).
                    </div>
                </div>
                ...
                <div class="explanation-step mt-16">
                    <div class="step-label">
                        Bước 2: Khấu trừ bảo hiểm <span class="ref">(5)</span>
                    </div>
                    <div class="step-desc">
                        Bảo hiểm = Lương đóng BH (hợp đồng) × (BHXH 8% + BHYT 1.5% + BHTN 1%) =
                        Lương đóng BH × 10.5%
                    </div>
                    <div class="step-calc">
                        Lương đóng BH ≈
                        {{ formatCurrency(Math.round(props.payroll.insuranceDeduction / 0.105)) }}
                        ₫<br />
                        Bảo hiểm =
                        {{ formatCurrency(Math.round(props.payroll.insuranceDeduction / 0.105)) }} ×
                        10.5% =
                        <span class="font-bold text-danger"
                            >{{ formatCurrency(props.payroll.insuranceDeduction) }} ₫</span
                        >
                    </div>
                    <div class="step-note">
                        Tỷ lệ theo <em>Chính sách giảm trừ</em> hiệu lực tại đầu kỳ. Căn cứ là lương
                        BH trong hợp đồng, không phải lương Gross.
                    </div>
                </div>
                ...
                <div class="explanation-step mt-16">
                    ...
                    <div class="step-desc">
                        Tổng giảm trừ = Giảm trừ bản thân + (Số người phụ thuộc × Mức giảm
                        trừ/người)
                    </div>
                    <div class="step-calc">
                        Giảm trừ bản thân: <strong>11.000.000 ₫/tháng</strong><br />
                        Giảm trừ gia cảnh: <strong>{{ dependentCount }}</strong> người ×
                        <strong>4.400.000 ₫/người</strong>
                    </div>
                    <div class="step-note">
                        Số người phụ thuộc lấy từ <em>Hồ sơ thuế nhân viên</em> hiệu lực tại cuối
                        kỳ.
                    </div>
                </div>
                ...
                <div class="explanation-step mt-16">
                    ...
                    <div class="step-desc">
                        Thu nhập chịu thuế = Gross - Bảo hiểm - Giảm trừ bản thân - Giảm trừ gia
                        cảnh (≥ 0)
                    </div>
                    <div class="step-calc">
                        = {{ formatCurrency(props.payroll.grossSalary) }} -
                        {{ formatCurrency(props.payroll.insuranceDeduction) }} - Giảm trừ =
                        <span class="font-bold"
                            >{{ formatCurrency(props.payroll.taxableSalary) }} ₫</span
                        >
                    </div>
                </div>
                ...
                <div class="explanation-step mt-16">
                    <div class="step-label">
                        Bước 5: Thuế TNCN lũy tiến <span class="ref">(6)</span>
                    </div>
                    <div class="step-desc">
                        Công thức: Thuế = (Thu nhập chịu thuế - Cận dưới bậc) × Thuế suất + Giảm trừ
                        nhanh
                    </div>
                    <div class="tax-bracket-table">
                        <table>
                            ...
                            <tbody>
                                <tr
                                    :class="
                                        props.payroll.taxableSalary > 0 &&
                                        props.payroll.taxableSalary <= 5000000
                                            ? 'active-bracket'
                                            : ''
                                    "
                                >
                                ...
                                <tr
                                    :class="
                                        props.payroll.taxableSalary > 5000000 &&
                                        props.payroll.taxableSalary <= 10000000
                                            ? 'active-bracket'
                                            : ''
                                    "
                                >
                                ...
                                <tr
                                    :class="
                                        props.payroll.taxableSalary > 10000000 &&
                                        props.payroll.taxableSalary <= 18000000
                                            ? 'active-bracket'
                                            : ''
                                    "
                                >
                                ...
                                <tr
                                    :class="
                                        props.payroll.taxableSalary > 18000000 &&
                                        props.payroll.taxableSalary <= 32000000
                                            ? 'active-bracket'
                                            : ''
                                    "
                                >
                                ...
                                <tr
                                    :class="
                                        props.payroll.taxableSalary > 32000000 &&
                                        props.payroll.taxableSalary <= 52000000
                                            ? 'active-bracket'
                                            : ''
                                    "
                                >
                                ...
                                <tr
                                    :class="
                                        props.payroll.taxableSalary > 52000000 &&
                                        props.payroll.taxableSalary <= 80000000
                                            ? 'active-bracket'
                                            : ''
                                    "
                                >
                    ...
                    <div class="step-calc mt-8">
                        Thu nhập chịu thuế:
                        <strong>{{ formatCurrency(props.payroll.taxableSalary) }} ₫</strong><br />
                        Thuế TNCN =
                        <span class="font-bold text-danger"
                            >{{ formatCurrency(props.payroll.pitTaxAmount) }} ₫</span
                        >
                    </div>
                </div>
                ...
                <div class="explanation-step mt-16">
                    ...
                    <div class="step-desc">
                        Thực nhận = Lương Gross - Bảo hiểm <span class="ref">(5)</span> - Thuế TNCN
                    ...
                    <div class="step-calc">
                        ...
                        <span class="font-bold text-success" style="font-size: 1.1rem"
                            >{{ formatCurrency(props.payroll.netSalary) }} ₫</span
                        >
                    </div>
                </div>
            </div>
